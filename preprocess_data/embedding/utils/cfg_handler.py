...
# Optional wandb (so converter jobs don't die if wandb/pydantic is weird)
try:
...
class train_config:
    ...
    def get_hyperparam(self, key: str, default=None):
        return self.cfg.get("hyperparameters", {}).get(key, default)

    def get_trdata_cfg(self, key: str, default=None):
        return self.cfg.get("data", {}).get(key, default)
    ...
    def hp(self, key: str, default=None):
        ...
        Priority:
        1) wandb.config (if wandb is available)
...
class data_config:
    ...
    def get_file_label_map(self):
        """Return list of (file_path, class_label) tuples from the config."""
    ...
    def get(self, key: str, default=None):
        return self.cfg.get("data_processing", {}).get(key, default)
