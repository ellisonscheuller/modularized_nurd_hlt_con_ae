...
def save_results(results, stypes, args, method):
    save_dir = os.path.join('output/ood_scores/', args.out_dataset, args.project_name, args.exp_name)
...
def get_ood_energy(args, model, val_loader, epoch, log, method):
    ...
    model.eval()
    init = True
    log.debug("######## Start collecting OOD energy score ########")
    with torch.no_grad():
...
def get_id_energy(args, model, val_loader, epoch, log, method):
    ...
    model.eval()
    log.debug("######## Start collecting ID energy score ########")
    with torch.no_grad():
        for i, (images, labels, _) in enumerate(val_loader):
            images = images.to(device)
            _, outputs = model(images)
...
def eval_energy():
    ...
    directory = "checkpoints/{in_dataset}/{name}/".format(in_dataset=args.in_dataset, name=args.project_name)
    fileHandler = logging.FileHandler(os.path.join(directory, "info_val.log"), mode='w')
    ...
    if args.in_dataset == 'color_mnist':
        cpts_directory = "./checkpoints/{in_dataset}/{name}/{exp}".format(in_dataset=args.in_dataset, name=args.project_name, exp=args.exp_name)
    else:
        cpts_directory = "./checkpoints/{in_dataset}/{name}/{exp}".format(in_dataset=args.in_dataset, name=args.project_name, exp=args.exp_name)
    
    # for test_epoch in test_epochs:
    test_epoch = 0
    cpts_dir = os.path.join(cpts_directory, "checkpoint_main.pth.tar")
    checkpoint = torch.load(cpts_dir, map_location=device)
